...
def roles_required(roles):
    def decorator(view_func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # Verificar si al menos uno de los roles del usuario está en la lista de roles requeridos
            if not any(role in roles_usuario for role in roles):
...
def login():
    ...
    if user and check_password_hash(user['contrasena'], password):
        session['user'] = user  # Establecer la información del usuario en la sesión
        if user['rol'] == 'ADMIN':
...
def registro():
    if request.method == "POST":
        resultado_registro, status_code = usuario.registroUsuario(request.form)
        if status_code == 200:
            return redirect(url_for('registro'))  # Redirige a la ruta 'registro'
        else:
            return jsonify({"error": resultado_registro}), status_code
    else:
        usuarios_con_indice = list(enumerate(usuario.obtenerUsuarios(), start=1)) 
        return render_template('registroUsuarios.html', usuarios=usuarios_con_indice)


    
@app.route('/user/editar', methods=["POST"])
def editar_usuario():
    if request.method == "POST":
        ...
        resultado = usuario.editarUsuario(usuario_data)
        # Redirigir al usuario a la página de registro después de la edición
        return redirect(url_for('registro'))

@app.route('/user/eliminar', methods=["POST"])
def eliminar_usuario():
    if request.method == "POST":
        usuario_id = request.json.get('usuario_id')
        # Realizar la eliminación del usuario en la base de datos
        result = db.Usuarios.delete_one({"_id": usuario_id})
        
        if result.deleted_count == 1:
            return jsonify({"success": True, "message": "Usuario eliminado correctamente"}), 200
        else:
            return jsonify({"success": False, "message": "Error al eliminar el usuario"}), 400

    # Redirigir a la misma vista después de eliminar el usuario
    return redirect(url_for('registro'))
